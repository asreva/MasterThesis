...
!!! network adapted for TSNE interpretability !!!
...
class SiameseArteryAnalysis_Or_patient(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        x1_b = self.resnet18_without_head[:5](x1)
        x2_b = self.resnet18_without_head[:5](x2)
        features_resnet_level6 = torch.cat((x1_b, x2_b), dim=1)
        
        x1_c = self.resnet18_without_head[:6](x1)
        x2_c = self.resnet18_without_head[:6](x2)
        features_resnet_level12 = torch.cat((x1_c, x2_c), dim=1)
        ...
        return pred, features_resnet_level6, features_resnet_level12, features_resnet_level18, features_flatten_level
...
class MiPredArteryLevel_Or_with_patient(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        lad_pred, lad_resnet_level6, lad_resnet_level12, lad_resnet_level18, lad_flatten_level = self.resnet_lad(x_lad)
        lcx_pred, lcx_resnet_level6, lcx_resnet_level12, lcx_resnet_level18, lcx_flatten_level = self.resnet_lcx(x_lcx)
        rca_pred, rca_resnet_level6, rca_resnet_level12, rca_resnet_level18, rca_flatten_level = self.resnet_rca(x_rca)
        
        resnet_level6 = torch.stack([lad_resnet_level6, lcx_resnet_level6, rca_resnet_level6], dim=1)
        resnet_level12 = torch.stack([lad_resnet_level12, lcx_resnet_level12, rca_resnet_level12], dim=1)
        resnet_level18 = torch.stack([lad_resnet_level18, lcx_resnet_level18, rca_resnet_level18], dim=1)
        flatten_level = torch.stack([lad_flatten_level, lcx_flatten_level, rca_flatten_level], dim=1)
           
        # Patient has MI if any of the artery has MI
        pred = torch.stack([lad_pred, lcx_pred, rca_pred], dim=1)
        
        before_pred_level = pred
        ...
        if self.two_neur_out:
            ...
            return pred, resnet_level6, resnet_level12, resnet_level18, flatten_level, before_pred_level
