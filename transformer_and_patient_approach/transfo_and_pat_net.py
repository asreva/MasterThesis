...
class Transformer_Structure_PatientData(nn.Module):
    ...
    def __init__(self, dim_seq=3456, num_heads=3, dim_head=18, num_encoders=8, num_regions=4):
        ...
        self.order_embedding = nn.Parameter(torch.randn(1, num_regions+1, dim_seq)) # one more for patient data
        self.to_trans, self.to_seq = nn.Linear(dim_seq, dim_head), nn.Linear(dim_head, dim_seq)
        self.extend_patient_data = nn.Linear(10, dim_seq)
    ...
    def forward(self, img, x_patient):
        # Add patient information
        x_patient = self.extend_patient_data(x_patient).unsqueeze(1)
        x = torch.cat((img, x_patient), dim=1)
...
class transformer_network_patient(nn.Module):
    ...
    def __init__(self, train_configuration, in_channels=1, num_levels=4, f_maps=16, dim_hidden=3456, num_heads=3, dim_head=18,
        ...
        self.transformer_structure = Transformer_Structure_PatientData(dim_seq=dim_hidden, num_heads=num_heads,
    ...
    def forward(self, x, x_patient):
        ...
        x = self.transformer_structure(x, x_patient)
        if self.verbose:
...
class PatientLevelDNN_PatientData(nn.Module):
    """ 
