"""
Aim: Implement the datasetloader for the MI prediction with patches and patient data
...
class AttentionDataset():
    ...
    def __init__(self, df, basepath, train_configuration, test):
        ...
        self.patient_to_sjid_df = pd.read_csv(PATH_TO_NAME_TO_SJID_DF, index_col=0)
            
        self.patient_data_df = pd.read_csv(PATH_TO_PATIENT_DF)
        self.patient_data_df = self.patient_data_df.set_index("sjid")
        if train_configuration["normalise_patient"]:
            self.patient_data_df["age"] = (self.patient_data_df["age"]-62.488857029506136)/12.621293377407468
            self.patient_data_df["bmi"] = (self.patient_data_df["bmi"]-27.2068571635655)/4.439795166397547
            self.patient_data_df["eGFR_bln"] = (self.patient_data_df["eGFR_bln"]-83.73087065151759)/24.1583841403525
        self.patient_data_df = self.patient_data_df[['sex', 'age', 'bmi', 'diabetes_hist', 'smoking_bln', 'hypertension_hist', 'cholesterolemia_hist', 'prev_CVD', 'eGFR_bln', 'resuscitation', 'chf_code_proc']]
    ...
    def __getitem__(self,idx):
        ...
        sjid = self.patient_to_sjid_df.loc[patient_name].values[0]
            ...
            for i_view, view in enumerate(["view1", "view2"]):
                if available_arteries[i_artery]:
                    for i_sect, section in enumerate(["magenta", "yellow", "green", "brown"]):
                        ...
                        except (BlockingIOError, OSError): # if othe GPU was using this ressource, retry
                            print("Error in data access")
                            time.sleep(1)
                            box = torch.load(self.basepath+"/"+patient_name+"/"+artery+"_"+view+"_"+section+"_box.pt")
                            centerline = torch.load(self.basepath+"/"+patient_name+"/"+artery+"_"+view+"_"+section+"_centerline.pt")
        ...
        patient_data = torch.from_numpy(self.patient_data_df.loc[sjid].values)
        patient_data = patient_data.numpy().astype('float32')
        
        return (lad, lcx, rca), patient_data, mi, available_arteries
...
def get_data_loaders(train_configuration, cv_split):
    ...
    if train_configuration["test"]:
        # No CV possible, just extract randomly a part of the data
        df_train, df_valid = df_train_valid, df_test
