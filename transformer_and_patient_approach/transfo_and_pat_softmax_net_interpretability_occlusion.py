...
!!! Adapted for occlusion interpretability !!!
...
class ArteryLevelDNN_PatientData(nn.Module):
    ...
    def __init__(self, two_neur_out, patient_features, train_configuration, in_channels=1, num_levels=4, f_maps=16, dim_hidden=3456, num_heads=3, dim_head=18, num_encoders=8, num_linear=2, num_class=2, num_regions=4, verbose=False):
        super().__init__()

        self.artery_analyser = transformer_network_patient(train_configuration, in_channels=in_channels, num_levels=num_levels, f_maps=f_maps, dim_hidden=dim_hidden, num_heads=num_heads, dim_head=dim_head, num_encoders=num_encoders, num_linear=num_linear, num_class=num_class, num_regions=num_regions, verbose=verbose)
    ...
    def forward(self, imgs):
        ...
        analyse_view1 = self.artery_analyser(img_view1, self.patient_features)
        analyse_view2 = self.artery_analyser(img_view2, self.patient_features)
        ...
        self.artery_analyse = artery_analyse # save in the network to access afterward and select most usefull patch
        ...
        if self.two_neur_out:
            ...
            pred_classes[0, 0] = 1-artery_pred
            pred_classes[0, 1] = artery_pred
            return pred_classes
        else:
            return artery_pred
    
class PatientLevelDNN_PatientDataSoftmax_occ(nn.Module):
    """ 
    ...
    """
    def __init__(self, two_neur_out, patient_features, train_configuration, in_channels=1, num_levels=4, f_maps=16, dim_hidden=3200, num_heads=3, dim_head=18,
        ...
        self.lad_analyser = ArteryLevelDNN_PatientData(two_neur_out, patient_features, train_configuration, in_channels, num_levels, f_maps, dim_hidden, num_heads, dim_head, num_encoders, num_linear, num_class, num_regions, verbose)
        self.lcx_analyser = ArteryLevelDNN_PatientData(two_neur_out, patient_features, train_configuration, in_channels, num_levels, f_maps, dim_hidden, num_heads, dim_head, num_encoders, num_linear, num_class, num_regions, verbose)
        self.rca_analyser = ArteryLevelDNN_PatientData(two_neur_out, patient_features, train_configuration, in_channels, num_levels, f_maps, dim_hidden, num_heads, dim_head, num_encoders, num_linear, num_class, num_regions, verbose)
       
        self.patient_features = patient_features

        self.verbose=verbose

    def forward(self, lad_imgs, lcx_imgs, rca_imgs, patient_data):
        ...
        lad_analysis = self.lad_analyser(lad_imgs)
        lcx_analysis = self.lcx_analyser(lcx_imgs)
        rca_analysis = self.rca_analyser(rca_imgs)
