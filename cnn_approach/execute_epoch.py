...
def execute_one_epoch(net, data_loader, train_configuration, optimizer, criterion, scheduler, device, modify_net):
    ...
    for batch_idx, (data, target) in enumerate(data_loader):
        ...
        if modify_net:
            optimizer.zero_grad()
        pred, lad_pred, lcx_pred, rca_pred, x_lad_pair, x_lcx_pair, x_rca_pair = net(data)
        
        loss_crit, loss_arteries, loss_siam = compute_full_loss(pred, lad_pred, lcx_pred, rca_pred, x_lad_pair, x_lcx_pair, x_rca_pair, target, criterion, train_configuration)
